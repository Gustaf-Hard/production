$(document).ready(function() {
    ...
    $.getJSON("../../api/subject/", function (data) {
        $.each(data, function () {
            $('#subjects').append('<button id="subject_button' + this.id + '" type="button" class="btn btn-xs btn-primary" style="margin: 2px">' + this.subject_name + '</button>');

            var subjct_id = this.id;
            $('#subject_button' + this.id ).click(function(){
                $("#subject_concept_list").html('');
                // Hide the full concept list
                $("#all_subject_list").hide();

                // POPULATE THE LESSON LIST
                $.getJSON("../../api/concepts/?subject_id=" + subjct_id, function (data) {
                    $.each(data, function () {
                        $('#subject_concept_list').append('<a class="list-group-item lesson_item" id="lesson_item' + this.id  + '"><p><strong> ' + this.concept + '</strong></p><p><small>' + this.subject_name + '</small> </p> <span id="lesson_id" style="display: none">' + this.id + '</span></a>');

                    });
                });

                $("#subject_concept_list").show();
            });
    ...
    $.getJSON("../../api/concepts/missing_inflections/", function (data) {
        $.each(data, function () {
            $('#all_subject_list').append('<a class="list-group-item lesson_item" id="lesson_item' + this.id  + '"><p><strong> ' + this.concept + '</strong></p><p><small>' + this.subject_name + '</small> </p> <span id="lesson_id" style="display: none">' + this.id + '</span></a>');
            var id = this.id;
