$(document).ready(function() {
    ...
    $.getJSON("../../api/concepts/missing_inflections/", function (data) {
        $.each(data, function () {
            ...
            $('#lesson_item' + this.id ).click(function(){
                ...
                $('#similar_concepts, #similar_concepts_header').html('');
                $.getJSON("../../api/concepts/?concept_id="+id, function (data) {
                    $.each(data, function () {
                        $('#similar_concepts').append('<a href="#" id="similar_concept_' + this.id + '" class="list-unstyled"> <li><span class="label label-primary">' + this.concept + ' (' + this.subject_name + ')</span></li><sp>');
                        $('#similar_concepts_header').html('Import the content from these concepts:');

                        var similar_concept = this;
                        $('#similar_concept_' + this.id ).click(function(){

                            // Just to make each word on each row instead of commas.
                            //similar_concept.inflection = similar_concept.inflection.replace(/\,/g, '\n');
                            //similar_concept.synonyms = similar_concept.synonyms.replace(/\,/g, '\n');

                            // Add the content to the form.
                            $('#inflections_form_group textarea').val(similar_concept.inflection ? similar_concept.inflection.replace(/\,/g, '\n') : '');
                            $('#synonyms_form_group textarea').val(similar_concept.synonyms ? similar_concept.synonyms.replace(/\,/g, '\n') : '');
                            $('#explanation_form_group textarea').val(similar_concept.explanation);
    ...
    $('#explanation_form_group textarea').keyup(function(){
    ...
    function resetForms(){
        ...
        $('#write_concepts_info textarea').val('');
